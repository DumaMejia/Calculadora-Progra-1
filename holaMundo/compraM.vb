Public Class compraM
    ...
    Sub obtenerdatos()
        ...
        ComboBox4.DataSource = conexion.obtenerdatos().Tables("proveedor").DefaultView()
        ComboBox4.DisplayMember = "nombre"
        ComboBox4.ValueMember = "proveedor.Idproveedor"
    ...
    Private Sub eliminarBT_Click(sender As Object, e As EventArgs) Handles eliminarBT.Click
        If eliminarBT.Text = "Eliminar" Then
            If (MessageBox.Show("Esta seguro de borrar " + "este", " registro",
                ...
                For i = 0 To Nfilas - 1
                    fila = DataGridView1.Rows(i)
                    Valore(i, 0) = fila.Cells("IdDcompra").Value.ToString
                Next
                For i = 0 To Nfilas - 1
                    conexion.mantenimientoDcompra(New String() {Valore(i, 0)}, "eliminar")
                    DatosGrid()
                Next
                conexion.mantenimientoCompra(New String() {Me.Tag}, "eliminar")
                limpiarCampos()
                controlesInicio(True)
            End If
        Else
