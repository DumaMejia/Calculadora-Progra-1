Public Class DetalleVenta
    ...
    End Sub
    Sub obtenerdatos()
        datatable = conexion.obtenerdatos().Tables("Dventa")
        datatable.PrimaryKey = New DataColumn() {datatable.Columns("IdDVenta")}
    ...
    Private Sub SeleccionarDato()
        idc = DataGridView1.CurrentRow.Cells("IdDVenta").Value.ToString()
    End Sub
    ...
    Private Sub nuevoBT_Click(sender As Object, e As EventArgs) Handles nuevoBT.Click
        If nuevoBT.Text = "Nuevo" Then 'Nuevo
            ...
            limpiarCampos()
        Else 'Guardar
            Dim msg = conexion.mantenimientoDVenta(New String() {
    ...
    Private Sub eliminarBT_Click(sender As Object, e As EventArgs) Handles eliminarBT.Click
        If eliminarBT.Text = "Eliminar" Then
            If (MessageBox.Show("Esta seguro de borrar " + "este", " registro",
                ...
                conexion.mantenimientoDVenta(New String() {Me.Tag}, "eliminar")
                limpiarCampos()
            End If
        Else
            controlesInicio(True)
